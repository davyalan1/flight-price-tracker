...
def _seed_legacy_single_trip(conn) -> None:
    """Simulate a real pre-multi-trip settings table: only the old flat
    `trip.*`/`alerts.*` keys (plus every other section a real install would
    have), no `trips` key at all. Trip/alerts values are deliberately
    different from the shared fixture so a carry-over bug can't hide behind
    coincidentally-matching defaults.
    """
    legacy = {
        "trip.origin": "LAX",
